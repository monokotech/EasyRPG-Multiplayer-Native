...
void WriteDataFrameHeader(uint8_t opcode, size_t len, char* header_start) {
...
void WebSocket::GotData(std::string_view data) {
	if (m_buffer.size() + data.size() > MAX_FRAME_SIZE) {
...
void WebSocket::ProcessDataFrame(uint8_t opcode, char* buf, size_t len) {
	switch (opcode) {
	case 9: // Ping
